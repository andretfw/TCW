...
export const PRIVACY_NOTICE_CONTENT: Record<SiteLocale, string> = {
  en: `
    ...
    <p>Dream Support applications are submitted through TCW’s website. The application and uploaded files are stored in a private Netlify Blobs store and are available only through TCW’s restricted reviewer area. The submission alert contains only the application reference, language and submission time; it does not contain the applicant’s name, answers, photographs or medical evidence. Please do not send medical records through ordinary email.</p>

    <h3>5. Who receives data?</h3>
    <p>Dream Support application access is limited to authorised TCW reviewers who need it for assessment and administration. We use Netlify for website hosting, identity-based reviewer access and private data storage. Other parts of our operations may use Google, Supabase, PayPal, Better Giving, banks, cryptocurrency infrastructure, communications providers, accountants, legal advisers and security providers. We may disclose information to public authorities when legally required. Service providers may process data only for the agreed service or under their own legal obligations.</p>
    ...
    <h3>9. Security</h3>
    <p>Dream Support application answers and files are encrypted by the application using AES-256-GCM before storage. Reviewer access requires an invited account and an authorised role or approved email address. Private responses are marked not to be cached, uploaded file types and sizes are restricted, and public download links are not created. Netlify also protects stored data in transit and at rest. No internet transmission or storage system is completely secure, so please share only the information requested and use the designated form.</p>
  ...
  ro: `
    ...
    <p>Cererile Dream Support sunt trimise prin site-ul TCW. Cererea și fișierele încărcate sunt stocate într-un spațiu privat Netlify Blobs și sunt disponibile numai în zona restricționată pentru evaluatori TCW. Alerta de trimitere conține numai referința cererii, limba și momentul trimiterii; nu conține numele solicitantului, răspunsurile, fotografiile sau documentele medicale. Nu trimiteți documente medicale prin email obișnuit.</p>

    <h3>5. Cine primește datele?</h3>
    <p>Accesul la cererile Dream Support este limitat la evaluatorii TCW autorizați care au nevoie de date pentru evaluare și administrare. Utilizăm Netlify pentru găzduirea site-ului, accesul evaluatorilor pe bază de identitate și stocarea privată a datelor. Alte activități ale organizației pot utiliza Google, Supabase, PayPal, Better Giving, bănci, infrastructură pentru criptomonede, furnizori de comunicații, contabili, consultanți juridici și furnizori de securitate. Putem divulga informații autorităților publice când legea o cere. Furnizorii pot prelucra datele numai pentru serviciul convenit sau potrivit obligațiilor lor legale proprii.</p>
    ...
    <h3>9. Securitate</h3>
    <p>Răspunsurile și fișierele din cererile Dream Support sunt criptate de aplicație cu AES-256-GCM înainte de stocare. Accesul evaluatorilor necesită un cont invitat și un rol autorizat sau o adresă de email aprobată. Răspunsurile private nu pot fi stocate în cache, tipul și dimensiunea fișierelor sunt limitate și nu sunt create linkuri publice de descărcare. Netlify protejează, de asemenea, datele stocate în tranzit și în repaus. Nicio transmitere sau stocare online nu este complet sigură; transmiteți numai informațiile cerute și utilizați formularul desemnat.</p>
  ...
  es: `
    ...
    <p>Las solicitudes de Dream Support se envían a través del sitio web de TCW. La solicitud y los archivos cargados se guardan en un almacén privado de Netlify Blobs y solo están disponibles en el área restringida para revisores de TCW. El aviso de presentación solo contiene la referencia de la solicitud, el idioma y la hora; no contiene el nombre de la persona solicitante, respuestas, fotografías ni documentación médica. No envíes historiales médicos por correo electrónico ordinario.</p>

    <h3>5. ¿Quién recibe los datos?</h3>
    <p>El acceso a las solicitudes de Dream Support se limita a revisores autorizados de TCW que lo necesiten para evaluar y administrar la ayuda. Utilizamos Netlify para alojar el sitio web, gestionar el acceso de los revisores mediante identidad y almacenar datos de forma privada. Otras partes de nuestras operaciones pueden utilizar Google, Supabase, PayPal, Better Giving, bancos, infraestructura de criptomonedas, proveedores de comunicaciones, contables, asesores jurídicos y proveedores de seguridad. También podemos comunicar información a autoridades públicas cuando la ley lo exija. Los proveedores solo pueden tratar los datos para el servicio acordado o conforme a sus propias obligaciones legales.</p>
    ...
    <h3>9. Seguridad</h3>
    <p>Las respuestas y los archivos de Dream Support son cifrados por la aplicación mediante AES-256-GCM antes de almacenarse. El acceso de los revisores requiere una cuenta invitada y un rol autorizado o una dirección de correo aprobada. Las respuestas privadas se marcan para que no se almacenen en caché, se restringen los tipos y tamaños de archivo y no se crean enlaces públicos de descarga. Netlify también protege los datos almacenados en tránsito y en reposo. Ninguna transmisión o almacenamiento por Internet es completamente seguro; comparte solo la información solicitada y utiliza el formulario designado.</p>
