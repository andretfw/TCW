...
export const PRIVACY_NOTICE_CONTENT: Record<SiteLocale, string> = {
  en: `
    ...
    <p>Dream Support applications are submitted through TCW’s website. Application answers and reviewer records are encrypted by TCW’s application and stored in a private Netlify Blobs store. Uploaded diagnosis evidence and optional photographs are stored in TCW’s restricted Google Workspace Shared Drive. Netlify keeps only the private Google Drive file identifier inside the encrypted application record; no public Drive or Blob link is created. The submission alert contains only the application reference, language and submission time and does not contain the applicant’s name, answers, photographs or medical evidence. Please do not send medical records through ordinary email.</p>

    <h3>5. Who receives data?</h3>
    <p>Dream Support application access is limited to authorised TCW reviewers who need it for assessment and administration. We use Netlify for website hosting, identity-based reviewer access and encrypted application records, and Google Workspace/Drive for private medical documents and optional photographs. Other parts of our operations may use Supabase, PayPal, Better Giving, banks, cryptocurrency infrastructure, communications providers, accountants, legal advisers and security providers. We may disclose information to public authorities when legally required. Service providers may process data only for the agreed service or under their own legal obligations.</p>
    ...
    <p>Incomplete Dream Support drafts and their Google Drive uploads are scheduled for deletion after 24 hours. When an application is declined or a grant/application is marked closed, its medical evidence and encrypted application record are scheduled for secure deletion within 90 days. A reviewer may delete an application sooner when appropriate. Limited grant, payment, decision, consent and accounting records may need to be kept separately for periods required by tax, accounting, audit, limitation or other laws. Deletion may be paused where evidence is reasonably required for a dispute, safeguarding matter or legal obligation. Public stories remain within the permission given until removed or permission is withdrawn, subject to lawful archival or recordkeeping needs. Provider security logs follow the provider’s configured retention period.</p>
    ...
    <h3>9. Security</h3>
    <p>Dream Support application answers and the stored Google authorisation token are encrypted by the application using AES-256-GCM before private Netlify storage. Documents are transferred over encrypted connections into a restricted TCW Google Workspace Shared Drive. Reviewer access requires invited and approved accounts; no public file links are created. Private responses are marked not to be cached, file types and sizes are restricted, filenames stored in Drive are pseudonymous, and protected portal links redirect authorised reviewers to Google Drive preview rather than serving document bytes publicly. Netlify and Google also protect data in transit and at rest. No internet transmission or storage system is completely secure, so please share only the information requested and use the designated form.</p>
  ...
  ro: `
    ...
    <p>Cererile Dream Support sunt trimise prin site-ul TCW. Răspunsurile și evidențele evaluatorilor sunt criptate de aplicația TCW și stocate într-un spațiu privat Netlify Blobs. Documentul de confirmare a diagnosticului și fotografiile opționale sunt stocate în spațiul Google Workspace Shared Drive restricționat al TCW. Netlify păstrează numai identificatorul privat Google Drive în dosarul criptat; nu este creat niciun link public Drive sau Blob. Alerta de trimitere conține numai referința cererii, limba și momentul trimiterii și nu conține numele solicitantului, răspunsurile, fotografiile sau documentele medicale. Nu trimiteți documente medicale prin email obișnuit.</p>

    <h3>5. Cine primește datele?</h3>
    <p>Accesul la cererile Dream Support este limitat la evaluatorii TCW autorizați care au nevoie de date pentru evaluare și administrare. Utilizăm Netlify pentru găzduirea site-ului, accesul evaluatorilor și dosarele criptate ale cererilor, iar Google Workspace/Drive pentru documentele medicale private și fotografiile opționale. Alte activități ale organizației pot utiliza Supabase, PayPal, Better Giving, bănci, infrastructură pentru criptomonede, furnizori de comunicații, contabili, consultanți juridici și furnizori de securitate. Putem divulga informații autorităților publice când legea o cere. Furnizorii pot prelucra datele numai pentru serviciul convenit sau potrivit obligațiilor lor legale proprii.</p>
    ...
    <h3>9. Securitate</h3>
    <p>Răspunsurile cererilor Dream Support și tokenul de autorizare Google stocat sunt criptate de aplicație cu AES-256-GCM înainte de stocarea privată în Netlify. Documentele sunt transferate prin conexiuni criptate într-un Google Workspace Shared Drive restricționat al TCW. Accesul evaluatorilor necesită conturi invitate și aprobate; nu sunt create linkuri publice către fișiere. Răspunsurile private nu pot fi stocate în cache, tipurile și dimensiunile fișierelor sunt limitate, denumirile fișierelor din Drive sunt pseudonimizate, iar linkurile protejate din portal redirecționează evaluatorii autorizați către previzualizarea Google Drive în loc să publice fișierul. Netlify și Google protejează datele în tranzit și în repaus. Nicio transmitere sau stocare online nu este complet sigură; transmiteți numai informațiile cerute și utilizați formularul desemnat.</p>
  ...
  es: `
    ...
    <ul>
      ...
      <li><strong>Solicitudes de Dream Support:</strong> identidad y contacto, confirmaciones de mayoría de edad, país, deseo, información de costes, diagnóstico o tratamiento oncológico, un documento de verificación del diagnóstico, fotografías opcionales, opciones de consentimiento, información del acuerdo de ayuda y prueba de que la ayuda aprobada se utilizó según lo acordado.</li>
    ...
    <p>Las solicitudes de Dream Support se envían a través del sitio web de TCW. Las respuestas y los registros de revisión son cifrados por la aplicación de TCW y almacenados en un espacio privado de Netlify Blobs. La prueba del diagnóstico y las fotografías opcionales se almacenan en el Google Workspace Shared Drive restringido de TCW. Netlify conserva únicamente el identificador privado de Google Drive dentro del expediente cifrado; no se crea ningún enlace público de Drive o Blob. El aviso de presentación solo contiene la referencia, el idioma y la hora y no contiene el nombre de la persona solicitante, respuestas, fotografías ni documentación médica. No envíes historiales médicos por correo electrónico ordinario.</p>

    <h3>5. ¿Quién recibe los datos?</h3>
    <p>El acceso a las solicitudes de Dream Support se limita a revisores autorizados de TCW que lo necesiten para evaluar y administrar la ayuda. Utilizamos Netlify para alojar el sitio web, gestionar el acceso de los revisores y almacenar expedientes cifrados, y Google Workspace/Drive para los documentos médicos privados y las fotografías opcionales. Otras partes de nuestras operaciones pueden utilizar Supabase, PayPal, Better Giving, bancos, infraestructura de criptomonedas, proveedores de comunicaciones, contables, asesores jurídicos y proveedores de seguridad. También podemos comunicar información a autoridades públicas cuando la ley lo exija. Los proveedores solo pueden tratar los datos para el servicio acordado o conforme a sus propias obligaciones legales.</p>
    ...
    <h3>9. Seguridad</h3>
    <p>Las respuestas de Dream Support y el token de autorización de Google almacenado son cifrados por la aplicación mediante AES-256-GCM antes del almacenamiento privado en Netlify. Los documentos se transfieren mediante conexiones cifradas a un Google Workspace Shared Drive restringido de TCW. El acceso de los revisores requiere cuentas invitadas y aprobadas; no se crean enlaces públicos a los archivos. Las respuestas privadas se marcan para que no se almacenen en caché, se restringen los tipos y tamaños de archivo, los nombres guardados en Drive son seudónimos y los enlaces protegidos del portal redirigen a revisores autorizados a la vista previa de Google Drive en lugar de publicar los documentos. Netlify y Google también protegen los datos en tránsito y en reposo. Ninguna transmisión o almacenamiento por Internet es completamente seguro; comparte solo la información solicitada y utiliza el formulario designado.</p>
