...
import {ACTIVE_DREAM_CAMPAIGNS} from '@/lib/campaigns';
...
const copy = {
  ...
  ro: {
    ...
    verified: 'Donația a fost verificată și adăugată în progress bar.',
...
export default function SupportDreamPage() {
  ...
  const [raisedEur, setRaisedEur] = useState(0);
  ...
  const loadProgress = useCallback(async () => {
    try {
      ...
      setRaisedEur(Number(result.raisedEur || 0));
    } catch {
      // Keep the last verified value if the progress endpoint is temporarily unavailable.
    }
  }, [campaign.id]);
  ...
  return (
    <section className="pt-32 pb-24 bg-neutral-50 min-h-screen">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7 max-w-7xl mx-auto items-start">
          <article className="bg-white rounded-3xl overflow-hidden shadow-lg border border-neutral-100 flex flex-col">
            ...
            <div className="p-5 flex flex-col">
              ...
              <p className="text-xs text-neutral-500 mb-5">{text.privacy}</p>

              <div className="mb-5" aria-label={`${raisedEur} euros raised of ${campaign.goalEur}`}>
                <div className="flex items-center justify-between gap-3 mb-2 text-xs font-semibold">
                  <span className="text-brand-600">
                    €{raisedEur.toFixed(2).replace('.00', '')} {text.raised}
                  </span>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                >
                  ...
                  <input type="hidden" name="item_name" value={text.title} />
                  ...
                  <input
                    type="hidden"
                    name="return"
                    value={`https://tutticancerwarriors.org/${locale}/support-a-dream`}
                  />
                  <button
                    type="submit"
              ...
              <div className="mt-4 flex items-start justify-center gap-2 text-xs text-neutral-500 text-center">
                <Heart className="w-3.5 h-3.5 text-brand-500 shrink-0 mt-0.5" fill="currentColor" />
        ...
        <div className="fixed inset-0 z-[100] bg-black/65 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl max-h-[92vh] overflow-y-auto">
            <div className="p-6 border-b border-neutral-100 flex items-start justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-neutral-900">{text.cryptoTitle}</h2>
                <p className="text-sm text-neutral-600 mt-2 leading-relaxed">{text.cryptoHelp}</p>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 space-y-5">
              <div>
                <div className="text-sm font-bold text-neutral-700 mb-2">{text.asset}</div>
              ...
              {asset !== 'btc' && (
                <div>
                  <div className="text-sm font-bold text-neutral-700 mb-2">{text.destination}</div>
              ...
              <div>
                <div className="text-sm font-bold text-neutral-700 mb-2">{text.address}</div>
                <div className="flex items-center gap-2 p-3 bg-neutral-50 border border-neutral-200 rounded-xl">
                  <code className="text-xs text-neutral-700 break-all flex-1">{cryptoAddress}</code>
                  <button
                    type="button"
              ...
              <div>
                <label htmlFor="txHash" className="text-sm font-bold text-neutral-700 block mb-2">
                  {text.txHash}
                </label>
                <input
                  id="txHash"
              ...
              {verificationMessage && (
                ...
                >
                  {verificationState === 'success' && <CheckCircle2 className="w-5 h-5 shrink-0" />}
